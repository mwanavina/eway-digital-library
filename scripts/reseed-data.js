...
async function reseedData() {
  try {
    ...
    await sql`DELETE FROM download_logs`;
    ...
    await sql`
      ...
      ('CS101', 'Java Programming I', ${pid1}),
      ('CS102', 'Java Programming II', ${pid1}),
      ...
      for (let year = 2022; year <= 2025; year++) {
        for (let semester = 1; semester <= 2; semester++) {
          ...
          for (const examType of examTypes) {
            ...
            // Use real PDF link for Java Programming courses, placeholder for others
            let filePath = `/documents/${course.id}/${year}/exam-${year}-s${semester}-${examType.replace('-', '')}.pdf`;
            if (course.name.includes('Java Programming')) {
